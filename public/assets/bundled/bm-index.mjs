const e = 20,
  ...
  n = { prototype: { send: WebSocket.prototype.send }, CLOSED: WebSocket.CLOSED, CLOSING: WebSocket.CLOSING, CONNECTING: WebSocket.CONNECTING, OPEN: WebSocket.OPEN };
async function c() {
  const e = (await self.clients.matchAll({ type: "window", includeUncontrolled: !0 })).map(async (e) => {
    ...
    t = Promise.race([Promise.any(e), new Promise((_e, t) => setTimeout(t, 1e3, new TypeError("timeout")))]);
  try {
    return await t;
  } catch (e) {
...
class w extends EventTarget {
  constructor(e, t = [], r, a) {
    ...
    (this.channel = new MessageChannel()),
      ...
      r.sendMessage({ type: "websocket", websocket: { url: e.toString(), protocols: t, requestHeaders: a, channel: this.channel.port2 } }, [this.channel.port2]);
  }
  send(...e) {
        ...
        try {
          e.ready || (await e.init()),
            await (async (e, t, r) => {
              ...
              a.body instanceof ReadableStream || a.body instanceof ArrayBuffer ? o.call(t, { type: "fetch", fetch: a }, [a.body]) : o.call(t, { type: "fetch", fetch: a });
            })(a, r, e);
        } catch (e) {
          u(r, e, "fetch");
        }
...
class k {
  ...
  createWebSocket(e, t = [], _r, a) {
    try {
      e = new URL(e);
    } catch (_t) {
      throw new DOMException(`Faiiled to construct 'WebSocket': The URL '${e}' is invalid.`);
    }
    if (!g.includes(e.protocol)) throw new DOMException(`Failed to construct 'WebSocket': The URL's scheme must be either 'ws' or 'wss'. '${e.protocol}' is not allowed.`);
  ...
  async fetch(e, r) {
    ...
    if (c.protocol.startsWith("blob:")) {
      ...
      return (r.rawHeaders = Object.fromEntries(e.headers)), (r.rawResponse = { body: e.body, headers: Object.fromEntries(e.headers), status: e.status, statusText: e.statusText }), (r.finalURL = c.toString()), r;
    }
    for (let e = 0; ; e++) {
      const t = (await this.worker.sendMessage({ type: "fetch", fetch: { remote: c.toString(), method: a.method, headers: o, body: n || void 0 } }, n ? [n] : [])).fetch,
...
export { k as BareClient, m as BareMuxConnection, w as BareWebSocket, n as WebSocketFields, p as WorkerConnection, d as browserSupportsTransferringStreams, k as default, e as maxRedirects, f as validProtocol };
